...
import { ApprovalDetails } from 'src/app/models/models/approvaldetails.model';
import { DistinctUserRequestViewModel } from 'src/app/models/models/distinctUserRequestViewModel.model';
...
@Component({
  selector: 'app-card-tablee',
  templateUrl: './card-tablee.component.html',
  styleUrls: ['./card-tablee.component.css']
})
export class CardTableeComponent implements OnInit {
  @Input()
  get color(): string {
    return this._color;
  }
  ...
  username: string = '';
  distinctUserRequests: DistinctUserRequestViewModel[] = [];
  ...
  loadRequests() {
    ...
    if (user_id) {
      this.approvalService.getApprovalDetails(JSON.parse(user_id)).subscribe(
        requests => {
          this.distinctUserRequests = requests;
        ...
        error => {
          console.error('Error retrieving users requests:', error);
        }
      );
    }
  }
  viewDetails(approval_Request_Id: number, id: number) {
